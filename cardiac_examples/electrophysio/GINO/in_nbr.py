...
# this information is calculated beforehand using this script and saved at
# ./data/innbr/ folder.
...
def save_innbr(
    ...
    for i, file in enumerate(npy_files):
        ...
        input_geom = torch.tensor(case['arr_0'].item()['points'], dtype=torch.float)
        ...
        in_nbr = torch.where(dists_1 <= 0.5, 1., 0.).nonzero().T

        innbr_file = file.replace('/npz/', '/innbr/').replace('.npz', '.pt')
        torch.save(in_nbr, innbr_file)
        print(f'{i} in_nbr calculated in {default_timer() - starttime} seconds.')
    
    return None

save_innbr(folder_path='./cardiac_examples/electrophysio/data5/npz')