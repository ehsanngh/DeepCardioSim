...
class GINO(BaseModel, name="GINO"):
    ...
    def __init__(
        ...
        self.gno_out2 = IntegralTransform(
                    mlp_layers=[4, 64, 64, 1],
                    mlp_non_linearity=gno_mlp_non_linearity,
                    transform_type=out_gno_transform_type,
                    use_torch_scatter=gno_use_torch_scatter
        )
    ...
    def integrate_latent(self, in_p, out_p, latent_embed, edge_index):
        ...
        latent_embed = latent_embed.reshape((n_in, -1))
    ...
    def forward(self,  x, input_geom, latent_queries, edge_index, ada_in=None, **kwargs):
        ...
        in_p = in_p.view((batch_size, *grid_size, self.fno_in_channels))
