...
def load_dataset_gino(
    ...
    for file in npy_files:
        ...
        # First, check if in_nbr has been calculated and saved previously.
        if Path(innbr_file).exists():
            in_nbr = torch.load(innbr_file, weights_only=True)
        else:
            dists_1 = torch.cdist(
        ...
        data = BipartiteData(
            ...
            edge_index=in_nbr,
            y=y,
            label=file.split('_ID_')[1].split('.npy')[0]
            )
        meshes.append(data)
